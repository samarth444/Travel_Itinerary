...
public class FlightController {
    ...
    public String searchFlights(@RequestParam String from, @RequestParam String to, Model model) {
        ...
        if (flights.isEmpty()) {
            logger.warn("No flights found from '{}' to '{}'", from, to);
            model.addAttribute("error", "No flights found!");
        } else {
            model.addAttribute("flights", flights);
        }

        return "flights_results"; // Ensure flights_results.html exists
    }
    ...
    @PostMapping("/confirm")
    public String confirmBooking(@RequestParam String flightNumber, Model model) {
        logger.info("Confirmed booking for flight: {}", flightNumber);

        // You can fetch flight info and simulate saving the booking
        Optional<Flight> flight = flightService.findFlightByNumber(flightNumber);
        if (flight.isPresent()) {
        ...
        return "confirmation"; // confirmation.html should show the result
    }
}
